...
var BRIDGE_SCRIPT = path.join(PLUGIN_DIR, "pipeline_engine", "bridge.py");
var _process = null;
...
async function onStartup(context) {
  try {
    ...
    if (result.healthy) {
      ...
      console.log(
        `[pipeline] Event bus ready: ${stats2.total_events ?? 0} events, ${stats2.consumer_groups ?? 0} groups`
      );
    } else {
      console.error(`[pipeline] Health check failed: ${result.error}`);
    }
  } catch (err) {
    console.error(`[pipeline] Startup error: ${err.message}`);
  }
}
