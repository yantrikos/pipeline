/**
 * Pipeline — Durable Event Bus Plugin for OpenClaw
 *
 * Bridges to the Python PipelineEngine via persistent subprocess
...
const BRIDGE_SCRIPT = path.join(PLUGIN_DIR, "pipeline_engine", "bridge.py");
...
function ensureProcess(): ChildProcess {
  ...
  proc.on("close", (code: number | null) => {
    ...
    for (const [rid, { reject }] of _pendingRequests) {
      reject(new Error(`Pipeline bridge exited with code ${code}`));
    }
    ...
    try {
      ...
      setTimeout(() => {
        if (_pendingRequests.has(requestId)) {
          ...
          reject(new Error(`Pipeline bridge timeout for command: ${command}`));
        }
      }, 30000);
...
export async function onStartup(context: {
  ...
  try {
    ...
    if (result.healthy) {
      ...
      console.log(
        `[pipeline] Event bus ready: ${stats.total_events ?? 0} events, ${stats.consumer_groups ?? 0} groups`
      );
    } else {
      console.error(`[pipeline] Health check failed: ${result.error}`);
    }
  } catch (err) {
    console.error(`[pipeline] Startup error: ${(err as Error).message}`);
  }
}

export async function onShutdown(): Promise<void> {
  killProcess();
  console.log("[pipeline] Event bus shut down");
}
